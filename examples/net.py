...
from v2g import MiMOVSSwitch, MiMNode, MiM # TODO modify
...
class Mininet( object ):
    ...
    def addMiM(self, suffix, client, server, nodeName=None, switchName=None):
        """Add all the necessary to perform a MiM in a V2G communication
            name
            """
        # todo: controllare consistency con il resto se si mette da solo i nomi @Luca
        if nodeName is None:
            nodeName = "h" + suffix
        if switchName is None:
            switchName = "s" + suffix

        MiMhost = self.addHost(nodeName, cls=MiMNode)
        MiMswitch = self.addSwitch(switchName, cls=MiMOVSSwitch)
        mim = MiM(MiMhost, MiMswitch, client, server)

        # create the links (todo: pass net to the constructor? mmm)
        self.addLink(mim.client, mim.MiMswitch)
        self.addLink(mim.server, mim.MiMswitch)
        self.addLink(mim.MiMhost, mim.MiMswitch)
